...
int removeRecordLogically(FILE *pFile, long long key) {
...
void testCandidate(Record record, int bucketIndex,                      // pomocna funkcija za proveru da li se odredjeni slog moze
    ...
    long long m = transformKey(record.key);
...
int removeRecordPhysically(FILE *pFile, long long key) {
    FindRecordResult findResult = findRecord(pFile, key);
...
void removeAllRecordsLogically(FILE* pFile) {
    ...
    for (i = 0; i < B; i++) {
        readBucket(pFile, &current_bucket, i);

        for (j = 0; j < BUCKET_SIZE; j++) {
            current_record = current_bucket.records[j];

            if (current_record.status == ACTIVE){
                current_bucket.records[j].status = DELETED;
            }
        }

        saveBucket(pFile, &current_bucket, i);
    }
}
